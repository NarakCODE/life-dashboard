...
import { usePerformanceDashboard, usePerformanceFilters } from "@/lib/performance/performance-query"
...
import type { PerformanceDashboardResponse, PerformanceFilters } from "@/lib/performance/types"
...
export function PerformanceContent({}: PerformanceContentWrapperProps) {
  ...
  const [rangeId, setRangeId] = useState<"7d" | "30d" | "90d" | "custom">("30d")
  ...
  const { data: performanceData, isLoading, error } = usePerformanceDashboard(
    workspaceId,
    performanceQuery,
    !!workspaceId
  )
    ...
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center">
          <div className="text-lg font-semibold">Loading performance data...</div>
          <p className="text-sm text-muted-foreground">Fetching metrics from your workspace</p>
        </div>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="text-center">
          <div className="text-lg font-semibold text-destructive">Failed to load data</div>
...
function getDefaultStartDate(rangeId: string): string {
  ...
  const days = rangeId === "7d" ? 6 : rangeId === "30d" ? 29 : rangeId === "90d" ? 89 : 29
