...
export function ProjectsContent() {
  ...
  const isLoading = isWorkspacePending || (Boolean(workspaceId) && isProjectsPending)
  ...
  const removeFilter = (key: string, value: string) => {
    const next = filters.filter((filter) => !(filter.key === key && filter.value === value))
    setFilters(next)
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (!viewOptions.showClosedProjects) {
      list = list.filter((project) => project.status !== "completed" && project.status !== "cancelled")
    }
    ...
    for (const { key, value } of filters) {
      ...
      else if (normalizedKey.startsWith("priority")) prioritySet.add(normalizedValue)
      else if (normalizedKey.startsWith("tag")) tagSet.add(normalizedValue)
    }

    if (statusSet.size) {
      list = list.filter((project) => statusSet.has(project.status.toLowerCase()))
    }

    if (prioritySet.size) {
      list = list.filter((project) => prioritySet.has(project.priority.toLowerCase()))
    }

    if (tagSet.size) {
      list = list.filter((project) =>
        project.workstreams.some((workstream) => tagSet.has(workstream.name.toLowerCase())),
      )
    }
